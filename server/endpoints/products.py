...
def add_product(product_id: str, product_name: str, hierarchy_code: str,
                price: float, product_length: float, product_depth: float,
                product_width: float):
    ...
    if data:
        conn.close()
        raise HTTPException(status_code=422, detail=f"{product_id} already exists")

    cursor.execute(f"INSERT INTO Products VALUES('{product_id}', '{product_name}', \
                    '{hierarchy_code}', '{price}', '{product_length}', \
                    '{product_depth}', '{product_width}')")
    conn.commit()
    conn.close()
    return {"product_id": product_id, "product_description": product_name,
            "hierarchy_code": hierarchy_code, "price": price,
            "product_length": product_length, "product_depth": product_depth,
...
def update_product(product_id: str,
    ...
    if hierarchy_code is not None:
        update_values.append(f"hierarchy_code = '{hierarchy_code}'")
    if price is not None:
        update_values.append(f"price = '{price}'")
    if product_length is not None:
        update_values.append(f"product_length = '{product_length}'")
    if product_depth is not None:
        update_values.append(f"product_depth = '{product_depth}'")
    if product_width is not None:
        update_values.append(f"product_width = '{product_width}'")
    if not update_values:
        conn.close()
        raise HTTPException(status_code=422, detail=f"At least one field must be specified")

    cursor = conn.cursor()
    cursor.execute(f"UPDATE Products SET {', '.join(update_values)} WHERE product_id = '{product_id}'")
    conn.commit()
